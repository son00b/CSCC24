...
satisfy pred = PsrOf p
  where
    ...
    p _ = Nothing
-- Could also be:
-- satisfy pred = anyChar >>= \c -> if pred c then return c else empty
...
--   p <* q
-- = liftA2 (\x _ -> x) p q
-- This runs both p and q, then tells you the answer of p only.
--
--   p *> q
-- = liftA2 (\_ y -> y) p q
...
chainl1 getArg getOp = getArg >>= \a -> more a
  where
    more a = (getOp
              >>= \op -> getArg
              >>= \x -> more (op a x)
